...
| `audit_numbers.py` | Lists every default, module constant and numeric literal in the code cells (`constants.txt`), and matches every number quoted in the markdown against the stored outputs and then against `history.md` §3 (`prose_numbers.txt` lists the unmatched ones; `audit_numbers_summary.txt` the counts). It matches values, not provenance, so a number with few significant figures can match by coincidence. |
...
`after_e7/` holds `audit_numbers.py` re-run on the notebooks after `history.md` §2 T4. Before
