...
prose) of `agent/decisions.md`, run against commit 76df22e.

| File | What it is |
|---|---|
| `extract_cells.py` | Dumps every cell of both notebooks and each code cell's stored text output. |
| `audit_numbers.py` | Lists every default, module constant and numeric literal in the code cells (`constants.txt`), and matches every number quoted in the markdown against the stored outputs and then against `agent/history.md` §3 (`prose_numbers.txt` lists the unmatched ones; `audit_numbers_summary.txt` the counts). It matches values, not provenance, so a number with few significant figures can match by coincidence. |
...
```bash
.venv/bin/python agent/audits/2026-09-13/extract_cells.py OUT
.venv/bin/python agent/audits/2026-09-13/audit_numbers.py OUT agent/audits/2026-09-13
.venv/bin/python agent/audits/2026-09-13/verify_audit.py OUT/m05.py
```

## Second run, after E4 and E7

`after_e7/` holds `audit_numbers.py` re-run on the notebooks after `agent/history.md` §2 T4. Before
