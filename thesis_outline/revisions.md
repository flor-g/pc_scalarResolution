...
`sections_3-6.md` §§3.2–3.3 (R18, R19; task U9 of `agent/history.md` §13,
...
> Increase the word budget for sections3-6 from 2400 words to 3000 words. Update agent/agent.md for the
...
Fifth and sixth messages (O7; recorded in full in `agent/history.md` §5):
...
| R6 | The three uses of "realizability" are an open decision, `agent/decisions.md` **O7**. Until O7 is settled, the outline does not introduce the term in a new sense. | user (2026-09-13) |
...
| R12 | **The delta read-out's two criteria** (`agent/decisions.md` B8), under *some*: (a) mode(φ_S\*) − mode(ℓ₀) < 0, the delta analogue of the shift; (b) mode(φ_S\*) outside the cell of *all*. They mirror Part C's two conditions, and satisfying one need not mean satisfying the other. | user (2026-09-13) |
| R13 | **The V shape is discussed in the paper** (Q2). | user (2026-09-13) |
| R14 | **Q2, option (a).** §4.5 reports the V under both read-outs as a result: the shared right arm, the left arm that exists only under q, and the 20 cells where the conjunctions part. §5.1 does not use the V, and its lines 329–338 go. **No evidence for a missing level is derived from the V.** Budget: §4.5 240, §5.1 350, §4.3 70. The code edits that let the prose quote these numbers are open tasks in `agent/history.md` §6. | user (2026-09-13) |
...
Decisions in `agent/decisions.md` this plan relies on: A9–A11 (θ_u learned, start 0, timescale
...
**2026-09-17.** That rebuild is now task **U10** of `agent/history.md` §13, and it needs no new code: Text cell 4b and Code Cell 2b already print every row. The count moves to **three of five** (flat, Beta(3,1), delta-like), which under `agent/agent.md` §5.4 is a headline change and is blocking decision **P-6** there. The §4.4 entry in §4 below was written against the Λ = 8 table and is stale in the same way; U10 corrects it too.
...
- **Tier B, the O-corner bullet (2026-09-21, `agent/decisions.md` O9 settled).** Applied: the bullet now
...
- **§3.2, one bullet after "The price of a soft lexicon".** The commitment: ℓ₀ enters through g_L
  ...
  have entered at g_S instead; that is `agent/decisions.md` **A3**, and it is a choice, not a consequence.
...
- **Not a reopening.** Every finding behind this block is a reason **for** A3 as it stands
  (`agent/agent.md` §3.1); A3 gains a dated amendment (U2) and stays settled.
...
- **Line 222.** "Untempered literal posterior" → "untempered literal listener". Add one sentence
  ...
  literature, with no further explanation (DEC5 of `agent/history.md` §4).
...
- The same inference at θ\* = 1407.77 would take **3.98e8** steps, about **5.0 hours**, at a
  ...
  H9/H11/H12 of `agent/history.md` §16.)
...
**Two corrections the rewrite had to make**, both recorded in `agent/decisions.md` E16:
...
**Source.** `agent/audits/2026-09-13-scale-structure/` (script and `output.txt`).
...
- **Which θ\*.** A single predicate's θ\* needs an exposure ensemble, and that is undecided
  (`agent/decisions.md` O8). Two illustrative ensembles, the predicate alone and the predicate with its
...
Record `agent/history.md` §12; audit `agent/audits/2026-09-17-scale-classes/`;
...
- **Costs to weigh under O13.** H2's open-scale half needs a cut t that is neither endpoint nor
  ...
  independently.
- **Nothing is printed by a cell**, so every number here is class (e) under `agent/agent.md` §3.3. Quoting
...
`agent/history.md` §12 §§7–10. In outline:
...
- **Tasks (T0–T13).** Checkpoint; the data file; Code Cell F; the Appendix F markdown cell;
  structure and ToC; references; the coupling check; execution; `agent/agent.md`; `agent/decisions.md`; the
...
- **Not started.** No notebook cell, no outline line and no `agent/agent.md` row has been touched for this
  change.
...
- **"Not yet written" is stale.** The plan's *Consequence without the alternatives level* bullet
  ...
  parameter convergence that `agent/agent.md` §3.2 requires before citing him either way. §5.3 states its
...
> **Revised 2026-09-21 by `agent/decisions.md` A19.** When this item was written, no self-contained
> halting mechanism was known and the section's argument was built around that gap. There is one:
> **halting is by tolerance**, and it is implemented (`agent/history.md` §16
...
- **Consequence without the alternatives level.** A mechanism outside F̃ must stop or slow the
  ...
  before citing it either way (agent/agent.md §3.2). **Not yet written.**
...
`agent/audits/2026-09-13-delta-criteria/` (script and `output.txt`). It executes Code Cell 1 and Code
...
re-executed. The tasks are closed in `agent/history.md` §6.
...
item 2. Recorded under A9 in `agent/decisions.md`.

### O7. The three senses of "realizability" (settled, in `agent/decisions.md`)
...
| F15 (not within 0.1% after 5,000 updates), 145/145 | `agent/history.md` §3 (recorded scripts, not a cell) |
| Exposure-only θ\* (−11.2844, −44.1766, −65.7004, −28.4375) | Code Cell B, ALTERNATIVE SPACES |
| §5.2's re-examination: contributions, leaks, loadings, illustrative θ\* | `agent/audits/2026-09-13-scale-structure/output.txt` (an audit script, not a cell) |
| The scale classes against Xiang et al. (2022): the class profiles by image type, R² by class and pooled for the model and q_lit, the Λ ladder per class and its bracket, the image-type difference, the between-class gaps, the displacement from the elicited prior, and the κ parity at n = 4 | `main.ipynb` **Code Cell F**, `appendix_f_report` (printed since T2/T7, 2026-09-22; **no longer class (e)**). Reads `data/xiang_2022/xiang_items.csv` — the only file either notebook reads (`agent/agent.md` §2 coupling 10). Two absolute classes only: the relative class is not modelled (O14). The superseding audit, `agent/audits/2026-09-17-scale-classes/`, is kept as the derivation and is **not** a source the prose cites; what remains only there — the relative class's fit, the m = 3/m = 4 comparison, the per-image-type Λ — stays class (e) and reaches no prose. |
| Where ℓ₀ enters: the two couplings, the 2BᵀWℓ₀ identity, BᵀW1 and the constant component, the σ-limit table, Part D under both placements at Λ = 8 and 512 | `main.ipynb` **Code Cell D**, *Sec. 5: where ell_0 enters* (printed since U3, 2026-09-17; no longer class (e)). The audit that established them, `agent/audits/2026-09-17-ell0-placement/`, is kept as the derivation and is not the source the prose cites. |
...
`agent/history.md` §6, are closed.)

## 9. Other stale pointers found

- `agent/agent.md` §1 named the Desktop outlines. **Fixed 2026-09-13**: it now names `thesis_outline/`
  and lists the Desktop drafts as not maintained.
- `agent/composition guide.md` line 3 names `sections_3-5_outline.md` and `background_sections_outline.md`.
- `agent/decisions.md` B2 (line 275), O3 (line 456) and E4 (line 659) cite `sections_3-5_outline.md`. A
...
2026-09-11 — and nothing cross-checked it against §6. The lesson is the one §3.3 of `agent/agent.md`
...
64**, its scope is now declared at its head, and `agent/agent.md` §2 carries the three-way division as
...
- **BG9.** `agent/composition guide.md` line 3 still names `sections_3-5_outline.md` and
...
- **BG11.** `agent/agent.md` line 56 describes Appendix F as "Eqs. (F1)–(F2)". **Eq. (F3) was added at
...
the closing bridge and §1.8's bridge corrected. **The introduction (the background's Opening) rewritten** for C10 and
C11, with the semantic/pragmatic sub-question moved to the second question (user) there and in the
central claim; §2.6 answers **no** outright; the closing bridge states implicature without explicit
competition. Opening 245 → 320, background target 4,160 → 4,235. Record: `agent/history.md` §22.
...
| **Z6** | `agent/decisions.md` E17, I6, I10, B12 | E17 records the supersession; I6 gains printed evidence and still fails its class; I10 records where each block went and why; **B12's rationale is corrected** | APPLIED |
...
criterion the headline of the constructive claim. **Taken.** Recorded as `agent/decisions.md` **B12**,
...
clarified in the outline. Both are **paper** positions: `agent/decisions.md` B10 keeps positions out of the
notebooks, and **neither notebook is touched by this block**. The positions are recorded as
`agent/decisions.md` A16's 2026-09-23 findings (position 1) and as the new entry **B11** (position 2).
...
| **PP10** | `sections_3-6.md` §5.1 (460 → 560) and the central claim; `agent/decisions.md` B11 | The order of the two motivations for the alternatives level: complexity first because it depends on no prior data (R2 stands); the conjecture second, stated model-relatively; a shift after the level would be welcome, not required; and why the conjecture cannot be promoted — the literal baseline the shift is measured against is a modelling construct, not an observable. With the introspective source of the wanting named in one sentence | APPLIED |
| **PP9** | `main.ipynb` Code Cell 2, mirrored in `appendix_E.ipynb` E2 | The C6 gap PP3 opened: `delta_readout_report` now prints its mode criteria counted over **distinct priors** as well as over rows, in the form the q blocks use, so "one of five" is quotable. Both notebooks re-executed | APPLIED |
| **PP8** | `agent/decisions.md` A16, B8, **B11** | A16 gains the position and its four limits; B8 gains the Λ = 512 tally and the one printed row where the two position criteria part; B11 is position 2's entry | APPLIED |
